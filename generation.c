...
Generation createGenerationFromGen(ArrayOfPoint points, int size, Generation gen, int fromLastGen) { // posiible should just call an array of routes a generation
	Generation newGeneration;

	newGeneration.routes = (Route*)malloc(sizeof(Route) * size);
	newGeneration.count = 0;
	newGeneration.size = size;
	for (int i = 0; i < fromLastGen; i++)
	{
